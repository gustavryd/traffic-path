...
```mcp
__polytope__get_container_logs(container: traveller-api, limit: 50)
```
...
2. Add desired collections - Run this tool:
   ```mcp
   __polytope__run(module: "traveller-api-add-couchbase-collection", args: {"name": "collection-name"})
   ```
...
```mcp
__polytope__get_container_logs(container: traveller-api, limit: 100)
```
